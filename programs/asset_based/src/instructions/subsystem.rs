...
pub fn _add_issuers_subsystem(ctx: Context<AddSubsystemIssuer>, issuer: Pubkey) -> Result<()> {
    let wrapper_account = &mut ctx.accounts.wrapper_account;
    wrapper_account.id_issuers.push(issuer);
    Ok(())
}

pub fn _remove_issuer_subsystem(ctx: Context<DeleteSubsystemIssuer>) -> Result<()> {
    let wrapper_account = &mut ctx.accounts.wrapper_account;
...
pub fn _deposit_token_subsystem(ctx: Context<DepositTokensSubsystem>, amount: u64, decimals: u8) -> Result<()> {
...
pub fn _withdraw_token_subsystem(ctx: Context<WithdrawTokensSubsystem>, amount: u64, decimals: u8) -> Result<()> {
